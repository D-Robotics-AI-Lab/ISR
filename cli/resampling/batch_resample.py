...
def _gripper_value(ee_qpos: list[float], arm_qpos: list[float]) -> float:
    if len(ee_qpos) > 7:
        return float(ee_qpos[7])
    if len(arm_qpos) > 6:
        return float(arm_qpos[6])
    return 0.0
...
def process_episode(
    ...
    for frame in data["data"]:
        ...
        positions_list.append(ee_qpos[:3])
        gripper_list.append(_gripper_value(ee_qpos, arm_qpos))
        timestamps_list.append(float(states["left_arm"]["timestamp"]))
    ...
    gripper = np.array(gripper_list, dtype=float)
    ...
    output_data = {
        "info": data.get("info", {}),
        "text": data.get("text", ""),
        "data": [frames_by_idx[idx] for idx in selected_data_indices],
    }
